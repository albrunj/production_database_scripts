...
from CommonConfig import datetime
...
def ImportData(querypath):
    ...
    for filepath in filepaths:
        ...
        if BatchWaferShorthand!=None and len(BatchWaferShorthand)>0:
            if IsShorthandFormat(BatchWaferShorthand)and WAFERDICT['Shorthand']==None:
                ...
                try:
                    ...
                    print("Something wrong with Batch and Wafer extraction.")
                    continue
        ...
        if WAFERDICT['Segment'] is None:
            for regexstring in StripTestRegexString:
                try:
                    ...
                    print("Something wrong with TimeRegexString.")
                    continue   
        try: 
        ...
        for regexstring in TemperatureRegexString:
            try:
                ...
                print("Something wrong with TemperatureRegex.")
                continue
        for regexstring in LengthVoltageRegexString:
            try:
                ...
                print("Something wrong with LengthVoltageRegex.")
                continue
    ...
    #NODUPWAFERDICTLIST=FILLEDWAFERDICTLIST
    print('')
    print('ImportData.py Complete!') 
    #norprint('output from file '+str(WAFERDICT['Filename'])+': '+str(WAFERDICT['Batch'])+': '+str(WAFERDICT['Wafer'])+': '+str(WAFERDICT['Type'])+":"+ str(WAFERDICT['Component'])+":"+ str(WAFERDICT['ComponentCode']))
...
def FillMissingInfo(WAFERDICTLIST):
    ...
    for WAFERDICT in WAFERDICTLIST:
        ...
        if BatchSeemsRight and WaferSeemsRight:
            ...
            #If the itk db for some reason has no batches or wafers, instead try getting component code from Component
            if ComponentCode is None and ComponentSeemsRight:
                ComponentCode=Component2ComponentCode(Component)
                #If Vitaliy wants the report to report all batches together, i.e. the batch/wafer numbers aren't "real", uncommment the following
                if ComponentCode is not None:
                    Batch,Wafer=Component2BatchWafer(Component)
            else: 
                Component=ComponentTemp  
        if Batch==None and Wafer == None:
            try:
                (Batch,Wafer)=HMComponent2BatchWafer(Component)
            except:
                Batch=None
                Wafer = None
        if ComponentCode is None:
            errprint('Could not match this information to a wafer on the ITKDB',(Batch,Wafer,Component,Shorthand))
            """
 #           if ITK_COMPONENTTYPE != "SENSOR_H_TEST":
            Component,ComponentCode=BatchWafer2ComponentAndCode(Batch,Wafer)
            if Component is None and ComponentCode is None:
                Component=WAFERDICT['Component']
                ComponentCode=WAFERDICT['ComponentCode']
        """
...
def Component2ComponentCode(Component):
    ComponentCode=None
    compindlist=[i for i in range(0,len(COMPONENTIDDICTLIST)) if COMPONENTIDDICTLIST[i]['Component']==Component]
    if compindlist!=None:
        if len(compindlist)==1:
...
def BatchWafer2ComponentAndCode(Batch,Wafer):
    ...
    if compindlist!=None:
        if len(compindlist)==1:
            ...
            errprint('Wafer is not in the ITK Database!b',str([Batch,Wafer]))
        else:
...
#      templist.append(compressed)
        ...
        if WAFERDICT['TestType'] in [TestType_Thickness,TestType_Thickness_Halfmoons] and WAFERDICT['AvThickness'] is not None and float(WAFERDICT['AvThickness'])>0:
