...
'''
    ...
    import SENSOR_THICKNESS_Analysis_Halfmoons
    TestType=TestType_Thickness_Halfmoons
    AnalysisFunction=SENSOR_THICKNESS_Analysis_Halfmoons.Analyze_THICKNESS_HALFMOONS
    TEMPANALYZEDNEWLISTTESTDICTLIST,TEMPANALYZEDOLDLISTTESTDICTLIST=AnalyzeBatches(AnalysisFunction,TESTCOMPARISONDATA[TestType]['CurrentTests'],TESTCOMPARISONDATA[TestType]['HistoricalTests'])
    #print("TEMPANALYZEDOLDLISTTESTDICTLIST",TEMPANALYZEDOLDLISTTESTDICTLIST)
    TESTCOMPARISONDATA[TestType]['CurrentTests']=TEMPANALYZEDNEWLISTTESTDICTLIST
    TESTCOMPARISONDATA[TestType]['HistoricalTests']=TEMPANALYZEDOLDLISTTESTDICTLIST
...
def AnalyzeBatches(AnalysisFunction,NEWLISTTESTDICTLIST,OLDLISTTESTDICTLIST):
    ...
    for compareind in range(0,len(NEWLISTTESTDICTLIST)):
     ...
     #   ANALYZEDNEWLISTTESTDICTLIST=NEWLISTTESTDICTLIST
     #   ANALYZEDOLDLISTTESTDICTLIST=OLDLISTTESTDICTLIST
        for WAFERDICT in NEWLISTTESTDICTLIST: 
                ANALYZEDWAFERDICT=AnalysisFunction(WAFERDICT)
                ANALYZEDNEWCOMPTESTDICTLIST.append(ANALYZEDWAFERDICT)
        for WAFERDICT in OLDLISTTESTDICTLIST:
                ANALYZEDWAFERDICT=AnalysisFunction(WAFERDICT)
                ANALYZEDOLDCOMPTESTDICTLIST.append(ANALYZEDWAFERDICT)
        ANALYZEDNEWLISTTESTDICTLIST.append(ANALYZEDNEWCOMPTESTDICTLIST)
        ANALYZEDOLDLISTTESTDICTLIST.append(ANALYZEDOLDCOMPTESTDICTLIST)
...
def GenerateReportHTML(TESTCOMPARISONDATA,outputfile):
    ...
    #for BatchInd in range (0,len (NODUPWAFERDICTLIST['Batch'])):
    #print("TESTCOMPARISONDATA['TITLES']", TESTCOMPARISONDATA['TITLES'])
    #print("len TESTCOMPARISONDATA['TITLES']",len(TESTCOMPARISONDATA['TITLES']))
    #for TestType in TestTypesToAnalyze:
        #print("TESTType,", TestType)
    #print("TESTCOMPARISON,", TESTCOMPARISONDATA[TestType])
       # print((TESTCOMPARISONDATA['ATLAS18_MAIN_THICKNESS_V1']['HistoricalTests']))
    for BatchInd in range(0,len(TESTCOMPARISONDATA['TITLES'])):
        #print("batchind",BatchInd) 
...
def GenerateSummaryTable(TESTCOMPARISONDATA,BatchInd):
    ...
    WaferListForBatch=[]
    WaferListForBatchnew=[]
 
    BATCHDATAFORTABLETEMP={}
    BATCHDATAFORTABLETEMPnew={}
    #print("TESTCOMPARISONDATA",TESTCOMPARISONDATA['TITLES'])
    ...
    for TestType in DisplayFieldsForTestTypes.keys():
        ...
        for Test in BatchTestTypeHistoricalTests:
            #print("batch,",Test['Batch'],Test['Wafer'])
            if Test['Batch'] in TESTCOMPARISONDATA['TITLES']:# in range(0,len(TESTCOMPARISONDATA['TITLES'])):            
                Batchhis.append(Test)         
                BatchID=Test['Batch']
            if Test['Decision'] in ['Pass','SoSo','Fail','MeasErr']:
                #Test['Decision']=''+Test['Decision']+''
        ...
        BATCHDATAFORTABLETEMPnew[TestType]=Batchhis+BatchTestTypeCurrentTests
        WaferListForBatch.extend([i['Wafer'] for i in BATCHDATAFORTABLETEMP[TestType]])  
        WaferListForBatchnew.extend([i['Wafer'] for i in BATCHDATAFORTABLETEMPnew[TestType]])  
    WaferListForBatch=set(WaferListForBatch)
    WaferListForBatchnew=set(WaferListForBatchnew)
    #print("BATCHDATAFORTABLETEMPnew", BATCHDATAFORTABLETEMPnew)
    ...
    BATCHDATAFORTABLEnew={}
    for Wafer in list(WaferListForBatchnew):
        if Wafer!=None:
            BATCHDATAFORTABLEnew[Wafer]={}
            for TestType in DisplayFieldsForTestTypes.keys():
                BATCHDATAFORTABLEnew[Wafer][TestType]=[TestDict for TestDict in BATCHDATAFORTABLETEMPnew[TestType] if TestDict['Wafer']==Wafer]

    outputstr=""
    print('batch',BatchID)
    #print("waferlist,", WaferListForBatch)
    #print("batchlist,", BatchListForBatch)
    #print("Batchlistnew,", BatchListForBatchnew)
   # print ("batchdatafortable,",BATCHDATAFORTABLE)
    #print ("batchdatafortablenew,", BATCHDATAFORTABLEnew)
    outputstr+=tableheader
    #WaferListForBatchNEW=sorted ([('W'+str(i),int(i)) for i in list(BATCHDATAFORTABLE.keys())])
    WaferListForBatchnewNEW=sorted ([('W'+str(i),int(i)) for i in WaferListForBatchnew])
    #print("waferlistnewNEW,", WaferListForBatchnewNEW)

    #for WaferName,Wafer in WaferListForBatchNEW:
    for WaferName,Wafer in WaferListForBatchnewNEW:
