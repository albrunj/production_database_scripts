...
# VF, 2021-08-xx, changes:
# - verification of the verifyer
# - further checks against pre-pro and production data
# - checks for the N(bad strips), and N(bad consecutive strips)
#
#------------------------------------------------------------
...
#scriptVersion = 4 # after verifying the error-checking machinery and implementing additional checks
#scriptVersion = 5 # un-did the allowance for SN = "20USBLS..." (didn't work completely anyway)
#scriptVersion = 5 # added a check for the existing HPK data in DB, to bump up the run number
scriptVersion = 6 # fixed the problem with reporting Implant Shorts in Implant Opens field
...
def get_run_number(c,code,only_tests):
    runNo = 1
    # only consider info in DB iff the objects already exist there
    if not only_tests:
        return str(runNo)

    found, resp = get_testruns_4_comp(c,code)
    if not found:
        return str(runNo)
    ...
    for testrun in resp:
        ...
        if testrun_code == 'MANUFACTURING18':
            locNo = int(testrun['runNumber'])
            if locNo >= runNo:
                runNo = locNo + 1
            #print(" found locNo = " + str(locNo) + ", changing ours to " + str(runNo))
    return str(runNo)


def upload_manufacturer_testrun_results(c, data, comp_code, institution, only_tests):
    ...
    dataNA = ['O.L.', '-']
    # VF, 2021-10-02: to check the previous tests if needed
    runNo = get_run_number(c,comp_code,only_tests)
    ...
    dto_in = {
        ...
        'defects' : [
            ...
            {
                'name' : 'Implant Opens',
                'description' : data['DEFECT'].get('Implant Opens', '9-9999')
            },
            {
...
def reverify_data( force_upload, dictData ):
    ...
    for filename in dictData:
        ...
        if filename.split(".")[0] != valSN:
            logRVD.error(" filename <" + filename + "> != SN = <" + valSN + ">, exiting")
            retVal = False
            continue
        # VF, 2021-09-27: this fix didn't work -- the parsing/checking phase was ok, the upload was not
...
def process_database_steps(c, input_dir, institution, date_received, do_qa, only_tests, force_upload, dummy, dictData, flags ):
    ...
    for filename in dictData:
        ...
        success, resp = upload_manufacturer_testrun_results(c, data, comp_code, institution, only_tests)
        if success: 
...
def main(args):
    ...
    log_base_file = "DBregiLOG_" + institution + "_"
