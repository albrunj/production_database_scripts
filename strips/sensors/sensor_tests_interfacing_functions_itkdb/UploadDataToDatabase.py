    ...
    for Test in UPLOADTESTLIST:
        ...
        if (('uuAppErrorMap')[0]!='cern-itkpd-main/uploadTestRunResults/'):
           ...
           # attachmentresult = client.post("createTestRunAttachment", data=dataforuploadattachment, files= attachment)
            ...
            #Check for errors
            if (('uuAppErrorMap')=={}):
                print('Upload of Test and File Succesful!')
        ...
        if Test['TestType']=='ATLAS18_VIS_INSPECTION_V2':
            Test['TestRunCode']=result['testRun']['id']
            for aKey in Test:
                if ( aKey.startswith("ScratchPad") or
                    ...
                    if imageFile is not None:
                        if len(imageFile) > 0:
                            f = open( imageFile, "rb")
                            client.post( "createBinaryTestRunParameter", 
                                    data = { "testRun": Test['TestRunCode'],
                                            "parameter": aKey },
                                    files = dict(data=f)
                            )
                            f.close()
            #Check for errors
            if (('uuAppErrorMap')=={}):
                print('Upload of Test, File and Image was Succesful!')
            elif (('uuAppErrorMap')[0]=='cern-itkpd-main/createTestRunAttachment/'):
