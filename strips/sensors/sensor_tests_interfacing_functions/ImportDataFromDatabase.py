...
def ImportDataFromDatabase():
    ...
    for APITestDict in APITESTLIST:
        ...
        if re.search(r'[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z',DateTime) is not None:
            ...
            print("No time found in datetime received from  database, using  00:00:00 instead.")
    ...
    with open(APIpicklefilename, "wb") as fp:  # Pickling
        pickle.dump(FINALTESTLIST, fp)  # nosec

    ###Below not actually needed, just useful for debugging
    try:
        with open(APIpicklefilename, "rb") as fp:  # Unpickling
            TOTALAPITESTLIST = pickle.load(fp)  # nosec
    except:
        TOTALAPITESTLIST = []
