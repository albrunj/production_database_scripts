...
#----------------------------------------------------
# A script for uploading the sensor recovery information
# Version 1, 2021-07-27, by VF
# Version 2, 2021-08-27, by VF: added the recovery methods of "long term hv", and "repeat iv", at Paul Miyagaway's and Xavi's requests
# Version 3, 2021-09-10, by VF: fixed "baked"->"baking" in the comments (pointed out by Pavla), the comment about the images, and the default values for missing (optional) floating values for Temp and Duration 
#
# Actions:
...
#Institute: CA
#RunNumber: 3
# BatchWaferNo; SN; Method; Date; Time; Comments; Instrument; Temperature; Duration # 1st line with keywords
#VPX90001-W326; 20USBSX0000326; Baking        ; 14 Feb 2021; 13:57:00; N2 flow; Oven Model 1234 ; 160.5; 20
#VPX90001-W327; 20USBSX0000327; UV irradiation; 15 Feb 2021; ; ; Module UV bar; 23; 0.5
#VPX90001-W328; 20USBSX0000328; Dry Storage   ; 16 Feb 2021; 13:57:17; Regular storage; Dr.Storage F1-1200-6; 22; 1440
# 
# Data formats expected:
#  1) Date in the form of DD Mon YYYY
#  2) Time in the form of HH:MM:SS (but it could be omitted)
#  3) Method: baking/uv irradiation/dry storage
#  4) Duration: in [hours]
#
#----------------------------------------------------

standardKeys = ["BatchWaferNo", "SN", "Method", "Date",  # The alwasy necessary info
                "Time", "Comments", "Instrument",        # helpful info
                "Temperature", "Duration" ]              # helpful info

standardMethods = ["baking", 
                   "uv irradiation", 
                   "dry storage", 
                   "long term hv", 
                   "repeat iv",
                   "plasma cleaning"] # the standard classification of damages

# the file to write the JSON data if the upload is not successful
bad_dump = "dump_bad_upload.json"

# The test version we use now
testType = "ATLAS18_RECOVERY_V1"
...
def get_json_from_dict( tInst, tRunN, sData ):
    ...
    # 2021-09-09, VF: Cannot convert from None to float though...
    valTemp = float(sData["Temperature"]) if len(sData["Temperature"]) > 0 else None
    valDur  = float(sData["Duration"   ]) if len(sData["Duration"   ]) > 0 else None
    ...
    retJSON = {
        ...
        "properties": {
            ...
            "RUNNUMBER": int(tRunN),
            "INSTRUMENT": minData["Instrument"]
        },
        "results": {
            "METHOD"     : minData["Method"     ],
            "TEMPERATURE": valTemp,
            "DURATION"   : valDur
            #"TEMPERATURE": float(minData["Temperature"]),
            #"DURATION"   : float(minData["Duration"   ])
        }
    }
...
def simple_data_integrity_checks(aLine, aDict):
    ...
    # to check the Method classification
    myMethod = aDict["Method"]
    if len(myMethod) == 0:
        print("Do not see the Recovery Method listed, will exit")
        return False
    # will only compare lower-case versions
    cmpMethod = myMethod.lower()
    if cmpMethod not in standardMethods:
        print(" Got new/unknown Recovery Method = <" + myMethod + ">")
        print(" The authorized types are:")
        print(standardMethods)
        print(" Exiting.")
        return False
...
def upload_test_to_DB( c, inputRecoveryFile, dictData, tInst, tRunN ):
    oFile = inputRecoveryFile + ".log"
    oLog = open( oFile, 'w' )
...
#    print("   writeSeparateFiles = " + str(writeSeparateFiles) )
    ...
    if len(inputRecoveryFile) == 0 :
        print(" wrong input file description, exiting...")
        return

    fAbsPath = Path(inputRecoveryFile)
    ...
    # read the input
    with open(inputRecoveryFile,'r') as iFile:
    ...
    goodUpload = upload_test_to_DB( c, inputRecoveryFile, dictData, tInst, tRunN )
    if not goodUpload:
...
if __name__ == '__main__':
    ...
    aTemplate += "#  3) Method: baking/uv irradiation/dry storage/long term hv/repeat iv/plasma cleaning\n"
    aTemplate += "#  4) Duration: in [hours]\n"
    aTemplate += "# \n"
    aTemplate += "# \n"
    aTemplate += " BatchWaferNo; SN; Method; Date; Time; Comments; Instrument; Temperature; Duration # 1st line with keywords \n"
    aTemplate += "# \n"
    aTemplate += "# \n"
    aTemplate += "<semicolon-separated information for these fields> # only 1st 4 are necessary \n"
    aTemplate += "# \n"

    strDescription = "Processing an input File (ASCII) to upload the sensor Recovery data to DB\n\n"
    ...
    parser.add_argument('-i', '--inputFile',     dest = 'inputRecoveryFile', type = str, default = '', help = 'A text file with the handling damage information')
