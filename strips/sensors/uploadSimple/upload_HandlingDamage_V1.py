...
#----------------------------------------------------
# A script for uploading the handling damage information
...
#Institute: CA
#RunNumber: 3
#BatchWaferNo; SN; Result; Date; Time; Comments; Location; DamageType; Description # 1st line with keywords
#VPX32409-W00147; 20USBSS0000147; Pass; 07 Jan 2020; 01:12:00; Damage during early tests ; Segment 2, strips 10-11; scratch; probe scratch during test
#VPX32409-W00148; 20USBSS0000148; Fail; 10 Feb 2020; 23:15:00; Damage during early tests; GR Right; deep scratch ; probe scratch during test
#VPX32409-W00149; 20USBSS0000149; Fail; 01 Dec 2021; ; Handling issue by XYZ; Entire sensor; Break ; Dropped the sensor
# 
# Data formats expected:
#  1) Date in the form of DD Mon YYYY
#  2) Time in the form of HH:MM:SS (but it could be omitted)
#  3) Location (not checked): corner / edge / guard ring / segment No, strip(s) Numbers
#  4) Damage type (not checked):  scratch / break
#
#----------------------------------------------------
...
standardKeys = ["BatchWaferNo", "SN", "Result", "Date",  # The alwasy necessary info
                ...
                "Location", "DamageType", "Description"] # the 1st damage location
...
testType = "ATLAS18_HANDLING_DAMAGE_V1"
...
def get_json_from_dict( tInst, tRunN, sData ):
    ...
    retJSON = {
        ...
        "results": {
            "RESULT"      : sData  ["Result"     ],
            "LOCATION"    : minData["Location"   ],
            "DAMAGE_TYPE" : minData["DamageType" ],
...
def simple_data_integrity_checks(aLine, aDict):
    ...
    # NOT checking for the DamageType for this test (Don't have enough statistics to make the classifier)
    #
...
def upload_test_to_DB( c, inputHandlingDamageFile, dictData, tInst, tRunN ):
    oFile = inputHandlingDamageFile + ".log"
    oLog = open( oFile, 'w' )
...
#    print("   writeSeparateFiles        = " + str(writeSeparateFiles) )
    ...
    if len(inputHandlingDamageFile) == 0 :
        print(" wrong input file description, exiting...")
        return

    fAbsPath = Path(inputHandlingDamageFile)
    ...
    # read the input
    with open(inputHandlingDamageFile,'r') as iFile:
    ...
    goodUpload = upload_test_to_DB( c, inputHandlingDamageFile, dictData, tInst, tRunN )
    if not goodUpload:
...
if __name__ == '__main__':
    ...
    aTemplate += "#  3) Location (not checked): corner / edge / guard ring / segment No, strip(s) Numbers\n"
    aTemplate += "#  4) Damage type (not checked):  scratch / break \n"
    aTemplate += "# \n"
    aTemplate += "# \n"
    aTemplate += " BatchWaferNo; SN; Result; Date; Time; Comments; Location; DamageType; Description # 1st line with keywords \n"
    aTemplate += "# \n"
    aTemplate += "# \n"
    ...
    parser.add_argument('-i', '--inputFile',     dest = 'inputHandlingDamageFile', type = str, default = '', help = 'A text file with the handling damage information')
