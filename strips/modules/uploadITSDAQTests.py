...
# Created: 2019/01/17, Updated: 2020/01/27
...
from itk_pdb.ITSDAQTestClasses import ComponentNotFound, ResultsFolder
...
def main(args):
    ...
    try:
        ...
        (sctvar_folder_path, get_confirm, upload_files, results_folder_path, ps_folder_path, config_folder_path, data_folder_path, recursion_depth) = (
            args.sctvar_folder_path, args.get_confirm, args.upload_files, args.results_folder_path, args.ps_folder_path, args.config_folder_path, args.data_folder_path, args.recursion_depth)

        INFO('Launching ITkPDSession.')
        session = ITkPDSession()
        session.authenticate()

        results_folder = ResultsFolder(sctvar_folder_path, results_folder_path, ps_folder_path, config_folder_path, data_folder_path)
                ...
                if results_file.tests == []:
                    ...
                    for i in indices:
                        try:
                            ...
                            upload_these_files = (not get_confirm) and upload_files
                            if get_confirm:
...
if __name__ == '__main__':
    ...
    required.add_argument(dest = 'sctvar_folder_path', type = str, default = os.getenv('SCTDAQ_VAR', None), help = 'path to the SCTVAR directory (only required if -r, -P, -C, or -D are not specified)')
    ...
    optional.add_argument('-r', '--resPath', dest = 'results_folder_path', type = str, help = 'path to the results directory (or equivalent) if an SCTVAR directory structure is not present')
    ...
    optional.add_argument('-D', '--dataPath', dest = 'data_folder_path', type = str, help = 'path to the data directory (or equivalent) if an SCTVAR directory structure is not present')
