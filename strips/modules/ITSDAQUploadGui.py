...
# Created: 2019/02/28, Updated: 2020/04/06
...
from itk_pdb.ITSDAQTestClasses import ComponentNotFound
from itk_pdb.ITSDAQTestClasses import ResultsFolder

pp = PrettyPrinter(indent = 1, width = 200)

# Add a default style to the QPushButton tool tips
...
# Nice function for highlighting where we're at in the gui
def DEBUG(func):
    @wraps(func)
    def func_wrapper(*args, **kwargs):
        if _DEBUG:
...
class ITSDAQUploadGui__Header(QWidget):
    ...
    # Bring up the header ui (title and button)
    @DEBUG
    def __initUI(self):
...
class ITSDAQUploadGui__Step1(QWidget):
    ...
    # Set all of our default member variables (I think these should be all...)
    @DEBUG
    ...
    # Bring up a Qt window for discovering directories
    @DEBUG
    ...
    # Explore for a directory and add the path as text to member_QLineEdit
    @DEBUG
    ...
    # Once our sctvar/results/ps/config/data paths are set, generate a results folder, find the files, and (optionally) filter by a regex
    @DEBUG
    def __getResultsFolder(self):
        # Have to convert '' to None for instantiating ResultsFolder
        if self.results_folder_path == '':
        ...
        if self.ResultsFolder is None:
            self.ResultsFolder = ResultsFolder(self.sctvar_folder_path, results_folder_path, ps_folder_path, config_folder_path, data_folder_path, enable_printing = False)
        else:
            self.ResultsFolder.reset(self.sctvar_folder_path, results_folder_path, ps_folder_path, config_folder_path, data_folder_path, enable_printing = False)
        self.ResultsFolder.getFiles(depth = self.recursion_depth)
        self.ResultsFolder.filterFilesByRegex(regexes = self.regexes)

    # Confirm the selected options for Step 1 and get our results folder and signal the end of Step 1
    @DEBUG
    def __confirm(self):
        ...
        if os.path.exists(self.input__sctvar_folder_path.text()):
            ...
            return
        if self.input__results_folder_path.text() != '':
            if os.path.exists(self.input__results_folder_path.text()):
                self.results_folder_path = self.input__results_folder_path.text()
            else:
                QMessageBox.warning(self, 'Error', '/$(SCTDAQ_VAR)/results/ path does not exist: %s' % self.input__results_folder_path.text())
                return
        ...
        if self.input__config_folder_path.text() != '':
            if os.path.exists(self.input__config_folder_path.text()):
                ...
                return
        if self.input__data_folder_path.text() != '':
            if os.path.exists(self.input__data_folder_path.text()):
                self.data_folder_path = self.input__data_folder_path.text()
            else:
                QMessageBox.warning(self, 'Error', '/$(SCTDAQ_VAR)/data/ path does not exist: %s' % self.input__data_folder_path.text())
                return
    ...
    # Build the layout of the Step 1 UI
    @DEBUG
    def __initUI(self):
        ...
        label__sctvar_folder_path = QLabel('Enter the /$(SCTDAQ_VAR)/ path:')
        ...
        label__results_folder_path = QLabel('(Optional) Enter the /$(SCTDAQ_VAR)/results/ path:')
        label__results_folder_path.setStyleSheet('font-size: 12pt; color: black;')
        self.input__results_folder_path = QLineEdit(self)
        self.input__results_folder_path.setStyleSheet('font-size: 12pt; color: black;')
        explore__results_folder_path = QPushButton('Explore', self)
        explore__results_folder_path.clicked.connect(lambda: self.__setDirectory(self.input__results_folder_path, 'Explore for /$(SCTDAQ_VAR)/results/'))
        explore__results_folder_path.setAutoDefault(True)
        explore__results_folder_path.setToolTip('Explore for /$(SCTDAQ_VAR)/results/')
        explore__results_folder_path.setStyleSheet('font-size: 12pt; color: black;')
        ...
        label__data_folder_path = QLabel('(Optional) Enter the /$(SCTDAQ_VAR)/data/ path:')
        label__data_folder_path.setStyleSheet('font-size: 12pt; color: black;')
        self.input__data_folder_path = QLineEdit(self)
        self.input__data_folder_path.setStyleSheet('font-size: 12pt; color: black;')
        explore__data_folder_path = QPushButton('Explore', self)
        explore__data_folder_path.clicked.connect(lambda: self.__setDirectory(self.input__data_folder_path, 'Explore for /$(SCTDAQ_VAR)/data/'))
        explore__data_folder_path.setAutoDefault(True)
        explore__data_folder_path.setToolTip('Explore for /$(SCTDAQ_VAR)/data/')
        explore__data_folder_path.setStyleSheet('font-size: 12pt; color: black;')
        ...
        self.layout.addWidget(label__results_folder_path,       2, 0, 1, 2)
        self.layout.addWidget(self.input__results_folder_path,  2, 2, 1, 5)
        self.layout.addWidget(explore__results_folder_path,     2, 7, 1, 1)
        self.layout.addWidget(label__ps_folder_path,            3, 0, 1, 2)
        self.layout.addWidget(self.input__ps_folder_path,       3, 2, 1, 5)
        self.layout.addWidget(explore__ps_folder_path,          3, 7, 1, 1)
        self.layout.addWidget(label__config_folder_path,        4, 0, 1, 2)
        self.layout.addWidget(self.input__config_folder_path,   4, 2, 1, 5)
        self.layout.addWidget(explore__config_folder_path,      4, 7, 1, 1)
        self.layout.addWidget(label__data_folder_path,          5, 0, 1, 2)
        self.layout.addWidget(self.input__data_folder_path,     5, 2, 1, 5)
        self.layout.addWidget(explore__data_folder_path,        5, 7, 1, 1)
        self.layout.addWidget(label__recursion_depth,           6, 1, 1, 1)
        self.layout.addWidget(self.input__recursion_depth,      6, 2, 1, 1)
        self.layout.addWidget(self.input__get_confirm,          6, 3, 1, 2)
        self.layout.addWidget(self.input__upload_files,         6, 5, 1, 2)
        self.layout.addWidget(label__regexes,                   7, 1, 1, 1)
        self.layout.addWidget(self.input__regexes,              7, 2, 1, 5)
        self.layout.addWidget(self.confirm,                     7, 7, 1, 1)
    ...
    # Reset member variables to defaults and initialize
    @DEBUG
    def reset(self):
        self.__setDefaults()
        self.initialize()
...
class ITSDAQUploadGui__Step2(QWidget):
    ...
    def __fillTable(self):
        self.table__files.clearContents()
...
class ITSDAQUploadGui__Step3(QWidget):
    ...
    @DEBUG
    def __fillTable(self):
        self.table__tests.clearContents()
...
class ITSDAQUploadGui__Step4(QWidget):
    ...
    def __fillTable(self):
        ...
        for j, filetype in enumerate(files_to_upload.keys()):
            ...
            elif filetype == 'composite_summary_filename_SD':
                label = '.txt (composite) Strobe Delay Summary Filename'
            elif filetype == 'composite_summary_filename_TPG':
                label = '.txt (composite) 3PG Summary Filename'
            elif filetype == 'composite_summary_filename_RC':
                label = '.txt (composite) Response Curve Summary Filename'
            elif filetype == 'composite_summary_filename_NO':
                label = '.txt (Composite) Noise Occupancy Summary Filename'
            elif filetype == 'strobe_delay_root_filename':
                label = '.root Strobe Delay Filename'
            elif filetype == 'three_point_gain_root_filename':
                label = '.root 3PG Filename'
            elif filetype == 'response_curve_root_filename':
                label = '.root Response Curve Filename'
            elif filetype == 'no_root_filename':
                label = '.root Noise Occupancy Filename'
            else:
                label = filetype
    ...
    def __upload(self, upload_json = True, upload_these_files = True):
        ...
        try:
            if upload_these_files:
                if not self.ResultsFile[i]['testRunId']:
                    id_found = bool(self.ResultsFile.findTestRunId(i, ITkPDSession = self.ITkPDSession))
                else:
                    id_found = True
                if id_found:
                    ...
                    print('WARNING in ITSDAQUploadGui__Step4.__upload(...) : Trying to upload files, but we couldn\'t find the ID of the test run to upload the files to!')
                    status_files = False
            else:
                status_files = True
        except RequestException as e:
    ...
    def initialize(self):
        ...
        if self.ResultsFile is not None:
            ...
            try:
                ...
                QMessageBox.warning(self, 'Error', 'Component with serial number \"%s\" could not be found in the ITkPD -- run number %s cannot be uploaded!' % (self.ResultsFile[i]['JSON']['properties']['SERIAL_NUMBER'], self.ResultsFile[i]['JSON']['runNumber']))
