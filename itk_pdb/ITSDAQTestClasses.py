...
# Created: 2019/01/17, Updated: 2021/07/01
# Contributing: mbasso, jchen, mnorfolk
import copy
import os
import re
import time
...
class ResultsFile(object):
    ...
    __slots__ = ['results_file_path', 'tests', 'full_test', 'iter_index', 'enable_printing', 'countBack', 'countForward']
    ...
    # I'm not sure what the noise occupancy structure looks like, so the summary file splicing is kept general.
    # once the parsing gets to a test, the function counts back and finds %NewTest and counts forward finds the next %NewTest and then I splice the lines inbetween into a list.
    def SummarySplicer(self, lines, currentLine):
        self.countBack = 0
        self.countForward = 0
        while '%NewTest' not in lines[currentLine - self.countBack]:
            self.countBack += 1
        while '%NewTest' not in lines[currentLine + self.countForward]:
            self.countForward += 1
            if currentLine + self.countForward == len(lines):
                break
        return self
    ...
    def getTests(self, debug = False):
        ...
        # DEBUG
        if debug:
            print('DEBUG in ResultsFile.getTests() : looking in file: %s' % self.results_file_path)
        ...
        for i, line in enumerate(lines):
            ...
            # DEBUG
            if debug:
                print('DEBUG in ResultsFile.getTests() : looking at line: %s' % i)
            ...
            if line[0] == '%':
                ...
                if '%NewTest' in line:
                    ...
                    if len(tests) != test_number:
                        if tests[test_number]['JSON']['results'] == {}:
                            if debug:
                                print('DEBUG in ResultsFile.getTests() : previous NewTest yielded empty test[\'JSON\'][\'results\'] -- PURGING!')
                            del tests[test_number]

                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding NewTest at line: %s' % i)
                    tests.append({'JSON': {'properties': {}, 'results': {}}, 'extra_data': {'NewTest_EXTRA': {}, 'DAQ_INFO_EXTRA': {}, 'DCS_INFO': {}, 'SCAN_INFO': {},
                                    'identifiers': {}}, 'files_to_upload': {}, 'files_to_upload_FULL': {}, 'listData': {}, 'testRunId': None})
                    ...
                    tests[test_number]['JSON'].update({
                        ...
                        'date':             '-'.join(lines[i+6][17:-1].split('/')[::-1]) + 'T' + lines[i+19][1:-2] + '.000' + time.strftime('%z')[:-2] + ':' + time.strftime('%z')[-2:],
                ...
                ############################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding DAQ_INFO at line: %s' % i)
                ...
                ##################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding DCS_INFO at line: %s' % i)
                    ...
                    tests[test_number]['extra_data']['DCS_INFO'].update({
                        'T0':               float(lines[i+3].split()[0]) if lines[i+3].strip().split() != ['.', '.'] else '.',
                        'T1':               float(lines[i+3].split()[1]) if lines[i+3].strip().split() != ['.', '.'] else '.',
                        'VDET':             float(lines[i+5].split()[0]) if lines[i+5].strip().split() != ['.', '.'] else '.',
                        'IDET':             float(lines[i+5].split()[1]) if lines[i+5].strip().split() != ['.', '.'] else '.',
                        'VCC':              float(lines[i+7].split()[0]) if lines[i+7].strip().split() != ['.', '.'] else '.',
                        'ICC':              float(lines[i+7].split()[1]) if lines[i+7].strip().split() != ['.', '.'] else '.',
                        'VDD':              float(lines[i+9].split()[0]) if lines[i+9].strip().split() != ['.', '.'] else '.',
                        'IDD':              float(lines[i+9].split()[1]) if lines[i+9].strip().split() != ['.', '.'] else '.',
                        'time_powered':     lines[i+11][:-2] if lines[i+11].strip() != '.' else '.'
                    })
                ...
                ##################################################################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding SCAN_INFO at line: %s' % i)
                ...
                ###################################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding StrobeDelay at line: %s' % i)

                    # Read test type
                    tests[test_number]['JSON']['testType'] = 'STROBE_DELAY'

                    StrobeDelayStart, StrobeDelayFinish = self.SummarySplicer(lines, i).countBack, self.SummarySplicer(lines,i).countForward
                    tests[test_number]['listData']['SummaryFileList'] = lines[(i-StrobeDelayStart):(i+StrobeDelayFinish)]
                    ...
                    tests[test_number]['files_to_upload']['strobe_delay_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '_StrobeDelayPlot_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '_' + ''.join(tests[test_number]['JSON']['properties']['TIME'].split(':')) + '.pdf'
                    tests[test_number]['files_to_upload']['det_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '.det'
                    tests[test_number]['files_to_upload']['composite_summary_filename_SD'] = 'StrobeDelay_SummaryFile_' + tests[test_number]['JSON']['runNumber'] + '_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '.txt'
                    tests[test_number]['files_to_upload']['strobe_delay_root_filename'] = 'strun' + str(tests[test_number]['JSON']['runNumber'].split('-')[0]) + '_' + str(tests[test_number]['JSON']['runNumber'].split('-')[1]) + '.root'

                    # Increment test_number
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : the following test (test_number = %s) has been parsed:' % test_number)
                        print('')
                ...
                ######################################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding ThreePointGain at line: %s' % i)

                    # Read test type
                    tests[test_number]['JSON']['testType'] = 'THREE_POINT_GAIN'

                    TPGStart, TPGEnd = self.SummarySplicer(lines, i).countBack, self.SummarySplicer(lines,i).countForward
                    tests[test_number]['listData']['SummaryFileList'] = lines[(i-TPGStart):(i+TPGEnd)]
                    ...
                    tests[test_number]['files_to_upload']['three_point_gain_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '_RCPlot_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '_' + ''.join(tests[test_number]['JSON']['properties']['TIME'].split(':')) + '.pdf'
                    tests[test_number]['files_to_upload']['composite_summary_filename_TPG'] = 'ThreePointGain_SummaryFile_' + tests[test_number]['JSON']['runNumber'] + '_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '.txt'
                    tests[test_number]['files_to_upload']['three_point_gain_root_filename'] = 'strun' + str(tests[test_number]['JSON']['runNumber'].split('-')[0]) + '_' + str(tests[test_number]['JSON']['runNumber'].split('-')[1]) + '.root'
                ...
                #####################################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding ResponseCurve at line: %s' % i)

                    # Read test type
                    tests[test_number]['JSON']['testType'] = 'RESPONSE_CURVE'

                    RCStart, RCFinish = self.SummarySplicer(lines, i).countBack, self.SummarySplicer(lines,i).countForward
                    tests[test_number]['listData']['SummaryFileList'] = lines[(i-RCStart):(i+RCFinish)]
                    ...
                    tests[test_number]['JSON']['results'] = results
                    tests[test_number]['extra_data']['identifiers'].update({'stream0': identifiers[:number_of_chips], 'stream1': identifiers[number_of_chips:]})
                    tests[test_number]['files_to_upload']['response_curve_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '_RCPlot_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '_' + ''.join(tests[test_number]['JSON']['properties']['TIME'].split(':')) + '.pdf'
                    tests[test_number]['files_to_upload']['composite_summary_filename_RC'] = 'ResponseCurve_SummaryFile_' + tests[test_number]['JSON']['runNumber'] + '_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '.txt'
                    tests[test_number]['files_to_upload']['response_curve_root_filename'] = 'strun' + str(tests[test_number]['JSON']['runNumber'].split('-')[0]) + '_' + str(tests[test_number]['JSON']['runNumber'].split('-')[1]) + '.root'
                ...
                ############################
                    ...
                    if trim_type == -1:
                        ...
                        if debug:
                            print('DEBUG in ResultsFile.getTests() : adding Trim at line: %s' % i)
                        ...
                        tests[test_number]['files_to_upload']['trim_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '_tr-1_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '.trim'
                        tests[test_number]['files_to_upload']['mask_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '_tr-1_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '.mask'
                        if debug:
                            print('DEBUG in ResultsFile.getTests() : the following test (test_number = %s) has been parsed:' % test_number)
                            print('')
                            pp.pprint(tests[test_number])
                ...
                #######################################
                    ...
                    if debug:
                        print('DEBUG in ResultsFile.getTests() : adding NO at line: %s' % i)

                    # Read test type
                    tests[test_number]['JSON']['testType'] = 'NOISE_OCCUPANCY'

                    NOStart, NOFinish = self.SummarySplicer(lines, i).countBack, self.SummarySplicer(lines,i).countForward
                    tests[test_number]['listData']['SummaryFileList'] = lines[(i-NOStart):(i+NOFinish)]
                    ...
                    tests[test_number]['extra_data']['identifiers'].update({'stream0': identifiers[:number_of_chips], 'stream1': identifiers[number_of_chips:]})
                    tests[test_number]['files_to_upload']['no_filename'] = tests[test_number]['JSON']['properties']['SERIAL_NUMBER'] + '_NoScurve_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '_' + ''.join(tests[test_number]['JSON']['properties']['TIME'].split(':')) + '.pdf'
                    tests[test_number]['files_to_upload']['composite_summary_filename_NO'] = 'NoiseOccupancy_SummaryFilename_' + tests[test_number]['JSON']['runNumber'] + '_' + ''.join(tests[test_number]['JSON']['date'].split('T')[0].split('-')) + '.txt'
                    tests[test_number]['files_to_upload']['no_root_filename'] = 'strun' + str(tests[test_number]['JSON']['runNumber'].split('-')[0]) + '_' + str(tests[test_number]['JSON']['runNumber'].split('-')[1]) + '.root'
        ...
        # DEBUG
        if debug:
            print('DEBUG in ResultsFile.getTests() : %s tests found.' % len(tests))
            print('DEBUG in ResultsFile.getTests() : the following tests have been parsed:')
            print('')
            pp.pprint(tests)
            print('')
        ...
        # DEBUG
        if debug:
            print('DEBUG in ResultsFile.getTests() : file scanned successfully!')
    ...
    def finalizeTest(self, test_number, ITkPDSession, upload_files = False, ResultsFolder = None, depth = 0, full_test = False):
        ...
        # Duplicate the test (for the next function)
        self.tests[test_number]['JSON-OLD'] = copy.deepcopy(self.tests[test_number]['JSON'])
        ...
        if self.__checkSerialNumber(serial_number):
            ...
            self.tests[test_number]['JSON'].update({'component': component['code'], 'institution': component['currentLocation']['code']})
        ...
        # Else -- if it is not a valid serial number
            ...
            component_list = (ITkPDSession.doSomething(action = 'listComponentsByProperty', method = 'POST', data = {'project': 'S', 'componentType': 'HYBRID', 'propertyFilter': property_filter})
                             + ITkPDSession.doSomething(action = 'listComponentsByProperty', method = 'POST', data = {'project': 'S', 'componentType': 'MODULE', 'propertyFilter': property_filter}))
        ...
        if upload_files:
            ...
            if full_test and self.full_test['state']:
                ...
                if test_number - lower_index == 0:
                    self.tests[test_number]['files_to_upload_FULL']['det_filename'] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload']['det_filename'], folder_type = 'config', depth = depth)
                ...
                # We want the .trim/.mask files
                    ...
                    self.tests[test_number]['files_to_upload_FULL']['trim_filename'] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload']['trim_filename'], folder_type = 'results', depth = depth)
                    self.tests[test_number]['files_to_upload_FULL']['mask_filename'] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload']['mask_filename'], folder_type = 'results', depth = depth)

                # We want the post-trim response curve .pdf output file
                elif test_number - lower_index == 4:
                    self.tests[test_number]['files_to_upload_FULL']['response_curve_filename'] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload']['response_curve_filename'], folder_type = 'ps', depth = depth)
            ...
            # If it's not a FullTest, we'll assume we want to keep everything
                ...
                for filetype in self.tests[test_number]['files_to_upload'].keys():
                    if filetype in ['det_filename']:
                        folder = 'config'
                        self.tests[test_number]['files_to_upload_FULL'][filetype] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload'][filetype], folder_type = folder, depth = depth)
                        ...
                        folder = 'ps'
                        self.tests[test_number]['files_to_upload_FULL'][filetype] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload'][filetype], folder_type = folder, depth = depth)
                    elif filetype in ['strobe_delay_root_filename', 'three_point_gain_root_filename', 'response_curve_root_filename', 'no_root_filename']:
                        folder = 'data'
                        self.tests[test_number]['files_to_upload_FULL'][filetype] = ResultsFolder.findFile(filename = self.tests[test_number]['files_to_upload'][filetype], folder_type = folder, depth = depth)
                    # It doesn't need to 'look' for the composite summary files but I have kept to the same structure as the files it does need to look for
                    elif filetype in ['composite_summary_filename_SD', 'composite_summary_filename_TPG', 'composite_summary_filename_RC', 'composite_summary_filename_NO']:
                        self.tests[test_number]['files_to_upload_FULL'][filetype] = self.tests[test_number]['files_to_upload'][filetype]

    # Essentially undo the finalization of a test
    def resetTest(self, test_number):
        try:
            self.tests[test_number]['JSON'] = copy.deepcopy(self.tests[test_number]['JSON-OLD'])
            del self.tests[test_number]['JSON-OLD']
        except KeyError:
            pass
    ...
    def printSummaryOfTests(self):
        if self.enable_printing:
            ...
            for i, test in enumerate(self.tests):
                row = [str(i), test['JSON']['properties']['SERIAL_NUMBER'], test['JSON']['testType'], test['JSON']['date'].split('T')[0], test['JSON']['properties']['TIME'],
    ...
    def uploadJSON(self, test_number, ITkPDSession):
        output = ITkPDSession.doSomething(action = 'uploadTestRunResults', method = 'POST', data = self.tests[test_number]['JSON'])
        self.tests[test_number]['testRunId'] = output['testRun']['id']
        if self.enable_printing:
    ...
    def uploadFiles(self, test_number, ITkPDSession, upload_files = False, filetypes = None):
        if upload_files:
            ...
            #test_name = self.tests[test_number]['JSON']['testType']
            date = self.tests[test_number]['JSON']['date']
            Summary_List = self.tests[test_number]['listData']['SummaryFileList']
            # Iterate over the files to upload
            for filetype in files_to_upload.keys():
                # Check if we have actually found a filepath
                if files_to_upload[filetype] is not None:
                    ...
                    elif filetype == 'strobe_delay_root_filename':
                        description = 'ITSDAQ strobe delay scan root file, run number %s. Obtained on %s.' % (run_number, date)
                    elif filetype == 'three_point_gain_filename':
                        description = 'ITSDAQ 3PG scan PDF file, run number %s. Obtained on %s.' % (run_number, date)
                    elif filetype == 'three_point_gain_root_filename':
                        description = 'ITSDAQ 3PG scan root file, run number %s. Obtained on %s.' % (run_number, date)
                    ...
                    elif filetype == 'response_curve_root_filename':
                        description = 'ITSDAQ response curve scan root file, run mumber %s. Obtained on %s.' % (run_number, date)
                    ...
                    elif filetype == 'no_root_filename':
                        description = 'ITSDAQ noise occupancy scan root file, run number %s. Obtained on %s.' % (run_number, date)
                    else:
                        description = None
                    # We have different upload behaviour dependending on whether we are opening files or using the composite spliced ones
                    if filetype not in ['composite_summary_filename_SD', 'composite_summary_filename_TPG', 'composite_summary_filename_RC', 'composite_summary_filename_NO']:
                        fields = {  'data': (os.path.basename(files_to_upload[filetype]), open(files_to_upload[filetype], 'rb')),
                                    'type': 'file',
                                    'testRun': self.tests[test_number]['testRunId'],
                                    'title': os.path.basename(files_to_upload[filetype]),
                                    'description': description  }
                    else:
                        fields = {  'data': (files_to_upload[filetype], BytesIO(''.join(Summary_List))),
                                    'type':'file',
                                    'testRun': self.tests[test_number]['testRunId'],
                                    'title':files_to_upload[filetype],
                                    'description': description  }
                    data = MultipartEncoder(fields = fields)
                    ITkPDSession.doSomething(action = 'createTestRunAttachment', method = 'POST', data = data)
                    if self.enable_printing:
    ...
    # Find the test run ID for a test (in case we want to upload files to it)
    def findTestRunId(self, test_number, ITkPDSession):
        json = self.tests[test_number]['JSON']
        code = json.get('component', None)
        if not code:
            retval = None
        else:
            matching_tests = ITkPDSession.doSomething('listTestRunsByComponent', 'GET', {'component': code, 'testType': json['testType'], 'runNumber': json['runNumber']})
            if len(matching_tests) > 1:
                retval = matching_tests[0]['id']
            else:
                retval = None
        self.tests[test_number]['testRunId'] = retval
        return retval

# Define an object for representing the contents of our SCTVAR/results/ folder
class ResultsFolder(object):

    __slots__ = ['sctvar_folder_path', 'results_folder_path', 'ps_folder_path', 'config_folder_path', 'data_folder_path', 'files', 'iter_index', 'enable_printing']
    ...
    def __init__(self, sctvar_folder_path = None, results_folder_path = None, ps_folder_path = None, config_folder_path = None, data_folder_path = None, enable_printing = True):
        self.reset(sctvar_folder_path = sctvar_folder_path, results_folder_path = results_folder_path, ps_folder_path = ps_folder_path, config_folder_path = config_folder_path, data_folder_path = data_folder_path,
                    enable_printing = enable_printing)

    def reset(self, sctvar_folder_path = None, results_folder_path = None, ps_folder_path = None, config_folder_path = None, data_folder_path = None, enable_printing = True):
        self.sctvar_folder_path = sctvar_folder_path + ('/' if not sctvar_folder_path.endswith('/') else '')
        if results_folder_path is None and sctvar_folder_path is not None:
            self.results_folder_path = os.path.join(sctvar_folder_path, 'results/')
        else:
            self.results_folder_path = results_folder_path + ('/' if not results_folder_path.endswith('/') else '')
        if ps_folder_path is None and sctvar_folder_path is not None:
            self.ps_folder_path = os.path.join(sctvar_folder_path, 'ps/')
        else:
            self.ps_folder_path = ps_folder_path + ('/' if not ps_folder_path.endswith('/') else '')
        if config_folder_path is None and sctvar_folder_path is not None:
            self.config_folder_path = os.path.join(sctvar_folder_path, 'config/')
        else:
            self.config_folder_path = config_folder_path + ('/' if not config_folder_path.endswith('/') else '')
        if data_folder_path is None and sctvar_folder_path is not None:
            self.data_folder_path = os.path.join(sctvar_folder_path, 'data/')
        else:
            self.data_folder_path = data_folder_path + ('/' if not data_folder_path.endswith('/') else '')
        self.files = []
        self.iter_index = 0
        self.enable_printing = enable_printing
    ...
    def getFiles(self, depth = 0):
        ...
        if self.results_folder_path is None:
            print('WARNING in ResultsFolder.getFiles() : self.results_folder_path == None -- cannot find any files!')
            return
        extension = '.txt'
    ...
    def findFile(self, filename, folder_type, depth = 0):
        ignore_folders = ['IGNORE']
        regex = re.compile(r'^(?=.*' + r')(?=.*'.join(os.path.splitext(filename)[0][:-1].split('_')) + r').*' + os.path.splitext(filename)[1] + r'$')
        if folder_type == 'results':
            ...
            folder = self.data_folder_path
        else:
            print('WARNING in ResultsFolder.findFile() : arg \'folder_type\' must be one of [\'results\', \'ps\', \'config\', \'data\'], returning None: folder_type = \'%s\'' % folder_type)
            return None
        if folder_type is None:
            print('WARNING in ResultsFolder.findFile() : supplied folder type \'%s\' specified self.%s_folder_path which == None, returning None!' % (folder_type, folder_type))
            return None
        for root, dirs, files in os.walk(folder):
            dirs = [d for d in dirs if d not in ignore_folders]
            if folder == self.data_folder_path:
        ...
        # In case we can't find the file, return None
        return None
